...
class HypersimImageDepthNormalTransform:
    ...
    def __call__(self, image, depth):
        ...
        image = transforms.functional.resize(image, self.size, interpolation=Image.BILINEAR)
        depth = transforms.functional.resize(depth, self.size, interpolation=Image.NEAREST)
...
def get_hypersim_dataset_depth_normal(data_dir, resolution, random_flip, norm_type, truncnorm_min, align_cam_normal=False, split='train'):
    ...
    image_dir = os.path.join(split_dir, "im")
    gt_dir = os.path.join(split_dir, "gt")
    image_files = sorted([f for f in os.listdir(image_dir) if f.endswith(('.jpg', '.png'))])
    gt_files = []
    for f in image_files:
        base_name = os.path.splitext(f)[0] # 获取不带后缀的文件名
        gt_path = os.path.join(gt_dir, f"{base_name}.png") # 拼接上.png后缀
    ...
    def preprocess_hypersim(examples):
        ...
        depths = [Image.open(depth).convert("L") for depth in examples[depth_column]] # List[np.array()]
        ...
        for img, dep in zip(images, depths):
            train_img, train_dep = transforms(img, dep)
