...
def run_evaluation(pipeline, task, args, step, accelerator):
    ...
    def gen_depth(rgb_in, pipe, prompt="", num_inference_steps=1):
        ...
        with autocast_ctx:
            rgb_input = rgb_in / 255.0 * 2.0 - 1.0  #  [0, 255] -> [-1, 1] # 归一化RGB图像
            task_emb = torch.tensor([1, 0]).float().unsqueeze(0).repeat(1, 1).to(pipe.device) # 执行预测深度图的任务
            task_emb = torch.cat([torch.sin(task_emb), torch.cos(task_emb)], dim=-1).repeat(1, 1)
            pred_depth = pipe(
                            rgb_in=rgb_input, 
                            task_emb=task_emb,
        ...
        for _ in range(len(train_dataloader_hypersim)):
            ...
            with accelerator.accumulate(unet):
                ...
                target_latents = vae.encode(
                    ...
                    ).latent_dist.sample() # 拼接了目标深度图和彩色图，双重目标
